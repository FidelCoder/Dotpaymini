...
const { clearAccessToken, getAccessToken } = require("./authTokenCache");
...
async function initiateB2C({
  ...
  if (!initiatorName) {
    throw new Error("Missing M-Pesa initiator name (MPESA_B2C_INITIATOR_NAME or MPESA_INITIATOR_NAME).");
  }
  if (!securityCredential) {
    throw new Error("Missing M-Pesa security credential (MPESA_B2C_SECURITY_CREDENTIAL or MPESA_SECURITY_CREDENTIAL).");
  }
...
async function initiateB2B({
  ...
  if (!initiatorName) {
    throw new Error("Missing M-Pesa initiator name (MPESA_B2B_INITIATOR_NAME or MPESA_INITIATOR_NAME).");
  }
  if (!securityCredential) {
    throw new Error("Missing M-Pesa security credential (MPESA_B2B_SECURITY_CREDENTIAL or MPESA_SECURITY_CREDENTIAL).");
  }
